...
└─────────────────────────────────────────────────────────────────────────────┘
  ...
  └────┬─────┘                                                    └────┬─────┘
       ...
       │──────────────────────▶┌─────────────┐                         │
       │◀──────────────────────│  Pay API    │                         │
       │  Actions[] (walletRpc — method varies)                             │
       │                       └─────────────┘                         │
       │                                                               │
...
func confirmPayment(
    ...
    for action in actions {
        ...
        // rpc.method   - e.g., "eth_signTypedData_v4", "eth_sendTransaction"
        // rpc.params   - JSON-encoded parameters

        guard let paramsData = rpc.params.data(using: .utf8),
              let params = try JSONSerialization.jsonObject(with: paramsData) as? [Any] else {
        ...
        case "eth_sendTransaction":
            guard let txParams = params[0] as? [String: Any] else {
                throw PaymentError.invalidParams
            }
            let txHash = try await yourWallet.sendTransaction(txParams)
        ...
        case "personal_sign":
            ...
            let signature = try await yourWallet.personalSign(message)
            signatures.append(signature)
...
struct WalletRpcAction {
    ...
    let method: String     // e.g., "eth_signTypedData_v4", "eth_sendTransaction"
...
| Symptom | Cause | Solution |
|---------|-------|----------|
| Invalid signature error | Wrong method | Check `action.walletRpc.method` and dispatch to the correct handler for each method |
