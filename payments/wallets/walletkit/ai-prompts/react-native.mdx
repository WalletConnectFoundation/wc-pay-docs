...
LOADING → ERROR (if failed)
        → WEBVIEW_DATA_COLLECTION (if collectData.url exists) → CONFIRM → CONFIRMING → SUCCESS
...
```typescript
type Step = 'loading' | 'intro' | 'webviewDataCollection' | 'confirm' | 'confirming' | 'result';
...
3. **WebView Data Collection View** - WebView form (if `paymentOptions.collectData?.url` exists)
...
When `paymentOptions.collectData?.url` is present, display the URL in a WebView. The hosted form handles rendering, validation, and T&C acceptance.
