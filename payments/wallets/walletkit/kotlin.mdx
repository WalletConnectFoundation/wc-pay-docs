...
<Warning>
WalletConnect Pay via WalletKit is currently only available on React Native and requires `@walletconnect/react-native-compat` to be installed. Native Android support will be available soon.
</Warning>
...
Add WalletKit to your `build.gradle.kts`:

```kotlin
implementation("com.reown:walletkit:{latest}")
```

WalletConnectPay is automatically included as a dependency of WalletKit.

<Info>
Check the [GitHub releases](https://github.com/reown-com/reown-kotlin/releases) for the latest version.
</Info>
...
// Initialize WalletKit as usual
val initParams = Wallet.Params.Init(core = CoreClient)

WalletKit.initialize(
    params = initParams,
    onSuccess = {
        // WalletKit and Pay SDK are both ready
    },
    onError = { error ->
