...
response.collectDataAction?.let { collectAction ->
    ...
    if (url != null) {
        ...
        val webViewUrl = "$url?prefill=$prefillBase64"
...
When `collectDataAction.url` is present, display the URL in a WebView. The WebView handles form rendering, validation, and T&C acceptance.
        ...
        viewModelScope.launch {
            ...
            optionsResult.onSuccess { response ->
                ...
                actionsResult.onSuccess { actions ->
                    ...
                    // Step 4: Collect data via WebView if required
                    response.collectDataAction?.url?.let { webViewUrl ->
...
10. **WebView Data Collection**: When `collectDataAction.url` is present, display the URL in a WebView rather than building native forms. The WebView handles form rendering, validation, and T&C acceptance.
