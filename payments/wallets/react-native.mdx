...
WalletConnect Pay supports all EVM-compatible chains and Solana. When providing accounts, use the CAIP-10 format:

| Chain Type | CAIP-10 Format | Example |
|------------|----------------|---------|
| EVM chains | `eip155:{chainId}:{address}` | `eip155:1:0x123...` (Ethereum Mainnet) |
| Solana | `solana:{cluster}:{address}` | `solana:mainnet:ABC...` |

<Note>
Include accounts for all networks you want to support to maximize payment options for your users. Popular EVM chains include Ethereum (1), Polygon (137), Base (8453), Arbitrum (42161), and Optimism (10).
</Note>
