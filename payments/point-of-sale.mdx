...
The **WalletConnect POS SDK** extends the open WalletConnect protocol into the world of **in-person payments**, enabling seamless, crypto-native checkout experiences at physical points of sale.

With the same universal interoperability that connects wallets and dapps, the POS SDK allows **Payment Service Providers (PSPs)** and **merchants** to accept crypto payments across multiple chains and wallets, no hardware changes or fragmented integrations required.
...
This SDK bridges online crypto rails with in-person commerce, making **crypto payments** as frictionless as any traditional payment method.
...
<Steps>
    <Step title="Universal Rail" icon="globe">
        A single SDK for PSPs and merchants to **accept crypto payments across any WalletConnect-compatible wallet and supported network**. No separate wallet integrations or new hardware required.
    </Step>
    ...
    <Step title="Multi-Chain with Full Visibility" icon="ethernet">
        Supports **EVM, Solana, and Tron** from day one. Provides a standardized payment lifecycle that you can use for reconciliation and customer support:

        `[PaymentCreated, PaymentRequested, PaymentProcessing, PaymentSuccess, PaymentError]`.
    </Step>
</Steps>
...
**DTC Pay**, Square-like or Ingenico-style providers are examples of PSPs that can use the POS SDK to accept crypto and stablecoin payments.

- Reduce cost and complexity of integrating crypto and stablecoin payments  
- Gain transaction lifecycle visibility for **refunds and reconciliation**  
- Stay competitive as crypto and stablecoin adoption enters mainstream retail  
...
### Merchants
- Accept crypto from any wallet with no new hardware  
...
|-----------|----------|
| **POS SDKs** | Kotlin |
| **Assets** | Native tokens and Stablecoins (e.g., USDC, USDT, ETH, SOL, etc.) |
| **Networks at Launch** | EVM chains (Ethereum, Optimism, Base, etc.), Solana, Tron |
| **Wallet Coverage** | 500+ WalletConnect-compatible wallets |
| **Transaction Lifecycle** | Standardized events -> `PaymentCreated`, `PaymentRequested`, `PaymentProcessing`, `PaymentSuccess`, `PaymentError` |
